...
\begin{titlepage}
  ...
  {\fontsize{20}{22}\bfseries Étape 1 — Exploration du jeu de données\par}
...
\begin{tikzpicture}[
  ...
  method/.style={box, text width=2cm, minimum height=0.8cm},
  ...
  % Branches d'analyse au-dessus
  \node[box] (stats) {Statistiques};
  \node[box, right=of stats] (lex) {Lexique};

  % Flux principal horizontal en dessous
  \node[box, below=1.5cm of stats] (data) {\textbf{train.csv}};
  \node[box, right=of data] (clean) {Nettoyage};
  \node[box, right=of clean] (prep) {Pré-traitement};
  \node[box, right=of prep] (topic) {Topic\\Modeling};

  % Méthodes alignées horizontalement en bas
  \node[method, below=1.2cm of data] (m1) {Méthode 1\\lexique métier};
  \node[method, right=of m1] (m2) {Méthode 2\\TF-IDF};
  \node[method, right=of m2] (m3) {Méthode 3\\Word2Vec};
  \node[method, right=of m3] (m4) {Méthode 4\\LDA};

  \node[group, fit=(m1) (m2) (m3) (m4), label=below:\small\textbf{Comparaison des méthodes}] (methods) {};

  % Grille de flèches

  % --- Flèches d'entrée et de sortie ---
  % \draw[arrow] (input) -- (data.west) node[midway, above, align=center, font=\tiny] {};
  % \draw[arrow] (topic.east) -- (output) node[midway, above, align=center, xshift=10pt, yshift=10pt, font=\tiny] {Étape 2\\Analyse des sentiments \\des conversations};

  % Flux principal horizontal
  \draw[arrow] (data) -- (clean);
  \draw[arrow] (clean) -- (prep);
  \draw[arrow] (prep) -- (topic);

  % Flèches horizontales entre branches d'analyse
  \draw[arrow] (clean.north) -- ++(0, 0.6) -| (stats.south);
  \draw[arrow] (clean.north) -| (lex.south);

  % Flèches vers les méthodes
  \draw[arrow] (topic.south) -- ++(0, -0.6) -| (methods.north);

\end{tikzpicture}
\caption{Vue d'ensemble de l'étape 1 : exploration du jeu de données et extraction des sujets.}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.65\textwidth]{graph_longRep_avec_stopwords.png}
\hfill
\includegraphics[width=0.65\textwidth]{graph_longRep_lemma.png}
...
La première méthode consiste à définir \textbf{manuellement} un dictionnaire où chaque clé est un
...
\begin{table}[h]
\centering
\begin{tabularx}{0.80\textwidth}{m{5em}|cc||cc}
\toprule
\textbf{Thème} & \textbf{Mot} & \textbf{fréquence} & \textbf{Mot} & \textbf{fréquence}\\
\midrule
\multirow{3}{1em}{\textit{Anxiété}} & \textit{anxiety} & 315 & \textit{anxiety} & 700\\
                                    & \textit{stress} & 169 & \textit{fear} & 423\\
                                    & \textit{nervous} & 84 & \textit{stress} & 300\\
\midrule
\multirow{3}{1em}{\textit{Dépression}} & \textit{depression} & 224 & \textit{depression} & 393\\
                                      & \textit{depressed} & 122 & \textit{sad} & 120\\
                                      & \textit{sad} & 88 & \textit{depressed} & 54\\
\midrule
\multirow{3}{1em}{\textit{Sommeil}} & \textit{sleep} & 139 & \textit{trauma} & 205\\
                                  & \textit{nightmare} & 45 & \textit{abuse} & 188\\ 
                                  & \textit{tired} & 44 & \textit{ptsd} & 63\\
\midrule
\multirow{3}{1em}{\textit{Relations}} & \textit{relationship} & 486 & \textit{relationship} & 1933\\
                                & \textit{friend} & 369 & \textit{friend} & 544\\ 
                                & \textit{family} & 267 & \textit{family} & 536\\
\bottomrule
\end{tabularx}
\caption{Trois mots les plus fréquents pour les patients (gauche) et thérapeutes (droite) selon le thème}
\end{table}

\begin{figure}[h]
\centering
\includegraphics[width=0.60\textwidth]{hist_occurence_lexique.png}
\caption{Distribution des occurrences des termes du lexique chez les patients (bleu) et thérapeutes (jaune)}
\end{figure}
Depuis le graphe, on peut observer que les thérapeutes ose exprimer beaucoup plus souvent des termes liées à notre dictionnaire,
en particulier les termes liées aux \textbf{relations familliale et amicale}, une observation intéressante mais \textbf{très limitée}.
...
Nous allons utiliser la fonction \texttt{TfidfVectorizer} de la bibliothèque \textbf{scikit-learn}.
...
\caption{Histogramme des 10 mots les plus importants selon le score TF-IDF}
\end{figure}

\subsubsection{Clustering K-Means}
Nous allons devoir maintenant regroupper les mots similaires en clusters,
...
\subsubsection{Résultats \footnote{à noter que les thèmes décrit pour les patients et thérapeutes sont des observations visuelles et non généré par le modèle, 
celui-ci génère uniquement les groupes de mots.}}


\begin{longtable}{p{2.2cm}|p{3.8cm}|p{3.8cm}|p{4cm}}
\toprule
\textbf{Clusters} & \textbf{Patient} & \textbf{Thérapeute} & \textbf{Global} \\
\midrule
\endhead
...
les clusters 0 et 2 (dépression), \textbf{ce qui peut s'interpréter positivement} : la dépression peut
provenir de sources différentes (fatigue mentale vs. idées suicidaires). Ce détail révèle que
\textbf{les conversations ne sont pas aléatoires} — les thérapeutes adaptent leur stratégie de réponse
en fonction du type de détresse exprimée par le patient.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{scatter_cluster_tfidf.png}
\caption{Projection en 2 dimenssions des conversations dans l'espace TF-IDF avec les clusters K-Means colorés}
\end{figure}

\textbf{Attention}, le plan est applatit sur deux dimenssions, ce qui peut masquer des nuances présentes dans les dimensions originales.
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{scatter_cluster_word2vec.png}
\caption{Projection en 2 dimenssions des conversations dans l'espace Word2Vec avec les clusters K-Means colorés}
\end{figure}
\clearpage
% ── Méthode 4 ─────────────────────────────────────────────────────────────────

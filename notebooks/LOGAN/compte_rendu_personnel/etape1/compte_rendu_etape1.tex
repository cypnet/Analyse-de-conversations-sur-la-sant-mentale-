...
\begin{tikzpicture}[
  ...
  \node[box, below=of clean] (prep) {Pré-traitement};
  \node[box, below=of prep] (topic) {Topic\\Modeling};

  % Méthode de Nettoyage
  \node[method, left=of clean] (n1) {suppression\\ponctuation};
  \node[method, below=of n1, text width=2.5cm] (n2) {lemmatisation};
  \node[method, left=of n2] (n3) {tokenisation};
  \node[method, left=of n1] (n4) {filtrage\\stopwords};
  \node[group, fit=(n1) (n2) (n3) (n4), label=below:\small\textbf{Comparaison des méthodes}] (methods1) {};

  % Méthodes de Topic Modeling
  \node[method, right=of topic] (tm1) {Méthode 1\\Lexique métier};
  \node[method, right=of tm1] (tm2) {Méthode 2\\TF-IDF};
  \node[method, below=of tm1] (tm3) {Méthode 3\\Word2Vec};
  \node[method, below=of tm2] (tm4) {Méthode 4\\LDA};
  % décaler le groupe entier vers la gauche
  \node[group, fit=(tm1) (tm2) (tm3) (tm4), label=below:\small\textbf{Comparaison des méthodes}] (methods2) {};
  ...
  \draw[arrow] (clean.west) -- ++(-0.2,0) -- ++(0, 1.3) -| (methods1.north);
  \draw[arrow] (topic.east) -- ++(0.4,0) -- ++(0, 1.3) -| (methods2.north);

\end{tikzpicture}
\caption{\centering Vue d'ensemble de l'étape 1 : \break Exploration du jeu de données et extraction des sujets.}
